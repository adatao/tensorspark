...
class TensorSparkModel():
   ...
   def __init__(self):
      ...
      self.apply_gradients = self.optimizer.apply_gradients(gradient_holders)
   ...
   def get_gradients(self):
      return [tf.div(grad[0], tf.fill(grad[0].get_shape(), self.num_gradients)).eval(feed_dict=self.feed_dict) for grad in self.gradient_holders]
