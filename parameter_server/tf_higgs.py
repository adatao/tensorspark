# -*- coding: utf-8 -*-
"""
Created on Mon Jan 25 18:25:07 2016
...
def model2(x, num_hidden_units, keep_prob):
  ...
  W_fc3 = weight_variable([num_hidden_units, 1])
  b_fc3 = bias_variable([1])
  return tf.matmul(h_fc2_drop, W_fc3) + b_fc3

x = tf.placeholder(tf.float32, [None, 28], name = "x")
y = tf.placeholder(tf.float32, shape=[None, 1], name = "y")
y_pred = model2(x, 1024, 0.5)
quad_loss = tf.nn.l2_loss(y_pred - y)
train_op = tf.train.RMSPropOptimizer(1e-4, decay=0.9, momentum=0.0, epsilon=1e-10, use_locking=False, name='RMSProp').minimize(quad_loss)
